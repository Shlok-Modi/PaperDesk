...
/* ── STATE ──────────────────────────────────────────────────────── */
let holdings = [];   // [{symbol, exch, qty, avg_price, ltp}] — today's open positions
let cashBalance = 0;
...
let lastUnrealizedPnl = 0; // updated by renderHoldings
...
async function loadHoldings() {
  const res = await fetch('api/positions.php', { headers: { 'Authorization': `Bearer ${token}` } });
  const data = await res.json();
  if (!res.ok) throw new Error(data.error || 'Failed to load positions.');
  holdings = (data.positions || []).map(h => ({ ...h, ltp: h.avg_price }));
}
...
function renderHoldings() {
  holdingCount.textContent = `${holdings.length} position${holdings.length === 1 ? '' : 's'}`;

  if (holdings.length === 0) {
    holdingsBody.innerHTML = `<tr><td colspan="8" style="text-align:center; color:var(--text-3); padding:32px 20px;">
      No open positions today — head to the <a href="index.html" class="form-link">Watchlist</a> to place a trade.
    </td></tr>`;
  ...
  if (!displayed.length) {
    holdingsBody.innerHTML = `<tr><td colspan="8" style="text-align:center; color:var(--text-3); padding:32px 20px;">No positions match this filter.</td></tr>`;
  }
...
document.getElementById('exportPdfBtn').addEventListener('click', async () => {
  ...
  try {
    ...
    // api/orders.php already scopes to today's orders by default (using
    // the DB server's date, not the browser's), so only the EXECUTED
    // filter is needed here.
    const now = new Date();
    const orders = (data.orders || []).filter(o => o.status === 'EXECUTED');
    if (!orders.length) { showToast('No trades executed today to export.'); return; }
    ...
    doc.setFontSize(16);
    doc.text("PaperDesk — Today's Positions Report", 14, 18);
    doc.setFontSize(10);
    doc.setTextColor(100);
    doc.text(`Account: ${userName}`, 14, 25);
    doc.text(`Generated: ${nowStr}`, 14, 30);
    ...
    doc.setFontSize(10);
    doc.text(`Day's P&L (open positions): ${lastUnrealizedPnl >= 0 ? '+' : ''}Rs. ${fmt(lastUnrealizedPnl)}`, 14, 47);
    doc.text(`Cash Available: Rs. ${fmt(cashBalance)}`, 14, 53);
    ...
    doc.save(`paperdesk-positions-${new Date().toISOString().slice(0, 10)}.pdf`);
  } catch (err) {
...
setInterval(async () => { await refreshLivePrices(); renderHoldings(); }, 5000);
