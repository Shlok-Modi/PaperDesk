...
// POST { email } -> generates a 6-digit OTP, emails it to the user,
// and stores only its hash. Always returns the same generic message
...
$db   = getDB();
$stmt = $db->prepare('SELECT id, name FROM pd_users WHERE email = ?');
$stmt->execute([$email]);
$user = $stmt->fetch();
...
// Simple rate limit: no more than 1 OTP request per 60 seconds per user
$rl = $db->prepare(
    "SELECT id FROM pd_otp_resets WHERE user_id = ? AND created_at > NOW() - INTERVAL '60 seconds' ORDER BY created_at DESC LIMIT 1"
);
$rl->execute([$user['id']]);
if ($rl->fetch()) {
...
$ins = $db->prepare(
    'INSERT INTO pd_otp_resets (user_id, otp_hash, otp_expires_at) VALUES (?, ?, ?)'
);
$ins->execute([$user['id'], $otpHash, $expiresAt]);

$name = htmlspecialchars($user['name'] ?: 'there');
$html = <<<HTML
<div style="font-family:Arial,sans-serif;max-width:480px;margin:0 auto;padding:24px;">
  <h2 style="color:#111;">PaperDesk password reset</h2>
  <p>Hi {$name},</p>
...
$sent = sendMail($email, $user['name'] ?? '', 'Your PaperDesk reset code', $html);

if (!$sent) {
    // Don't leak SMTP details to the client; the error is already logged.
